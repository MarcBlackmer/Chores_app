...
COPY public.categories (id, cat_name) FROM stdin;
1	Dishes
3	cleaning
4	yard work
\.
...
COPY public.chores (id, chore_name, recurrence, category_id, user_id, status, points) FROM stdin;
1	Empty dishwasher	daily	1	5	incomplete	10
2	Clean bedroom	Daily	3	3	complete	5
\.
...
COPY public.users (id, user_name, user_role) FROM stdin;
1	marc	admin
2	tom	admin
3	katie	user
5	bob	user
...
ALTER TABLE ONLY public.chores
    ADD CONSTRAINT chores_chore_name_key UNIQUE (chore_name);
...
ALTER TABLE ONLY public.chores
    ADD CONSTRAINT chores_category_id_fkey FOREIGN KEY (category_id) REFERENCES public.categories(id);
...
ALTER TABLE ONLY public.chores
    ADD CONSTRAINT chores_user_id_fkey FOREIGN KEY (user_id) REFERENCES public.users(id);
